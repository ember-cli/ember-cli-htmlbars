  ...
  return plugins.some((plugin) => {
    if (Array.isArray(plugin)) {
      ...
      return (
        ...
        require.resolve('./require-from-worker')
      );
    } else {
      return false;
    }
...
function setup(pluginInfo, options) {
  ...
  let htmlbarsOptions = buildOptions(
    projectConfig,
    templateCompilerPath,
    pluginInfo,
  );
...
function makeCacheKey(templateCompilerPath, pluginInfo, extra) {
  let templateCompilerCacheKey =
    getTemplateCompilerCacheKey(templateCompilerPath);
  let cacheItems = [templateCompilerCacheKey, extra].concat(
    pluginInfo.cacheKeys.sort(),
  );
...
function setupPlugins(wrappers) {
  ...
  for (let i = 0; i < wrappers.length; i++) {
    ...
    dependencyInvalidation =
      dependencyInvalidation || wrapper.dependencyInvalidation;
    plugins.push(
      addDependencyTracker(wrapper.plugin, wrapper.dependencyInvalidation),
    );
