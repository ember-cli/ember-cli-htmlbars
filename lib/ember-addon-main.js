...
module.exports = {
  ...
  init() {
    ...
    this.logger = require('heimdalljs-logger')(
      'ember-cli-htmlbars:addon:' + parentName,
    );
  },
  ...
  _shouldColocateTemplates() {
    ...
    let hasValidEmberVersion =
      hasEmberSource && semver.gte(emberVersion, '3.13.0');
  ...
  setupPreprocessorRegistry(type, registry) {
    ...
    registry.add('template', {
      ...
      toTree(tree) {
        ...
        this._addon.logger.debug(
          `setup *.hbs compiler with ${htmlbarsOptions.pluginNames}`,
        );
        ...
        let checker = new VersionChecker(this._addon.project).for(
          'ember-cli',
          'npm',
        );
  ...
  included() {
    ...
    this._requiresModuleApiPolyfill =
      checker.exists() && checker.lt('3.27.0-alpha.1');
    ...
    if (!utils.isInlinePrecompileBabelPluginRegistered(babelPlugins)) {
      ...
      if (pluginInfo.canParallelize) {
        ...
        this.logger.debug(
          'Prevented by these plugins: ' + pluginInfo.unparallelizableWrappers,
        );
    ...
    if (
      this._shouldColocateTemplates() &&
      !utils.isColocatedBabelPluginRegistered(babelPlugins)
    ) {
      babelPlugins.push([
